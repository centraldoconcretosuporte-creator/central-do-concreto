import type { Metadata } from "next";
import { Eyebrow } from "@/components/Eyebrow";
import WhatsAppLink from "@/components/WhatsAppLink";

const SITE_URL = "https://centraldoconcretoentrega.com.br";
const WHATSAPP_URL = "https://wa.me/5551996691757";
const PAGE_URL = `${SITE_URL}/produtos/cobogos`;
const PAGE_TITLE = "Cobogós de Concreto em Ivoti-RS · Central do Concreto";
const PAGE_DESCRIPTION =
  "Cobogós de concreto pré-moldado em Ivoti-RS. Modelos variados pra paredes vazadas, fachadas e divisórias.";

export const metadata: Metadata = {
  title: PAGE_TITLE,
  description: PAGE_DESCRIPTION,
  alternates: { canonical: `${SITE_URL}/cobogos` },
  openGraph: {
    title: PAGE_TITLE,
    description: PAGE_DESCRIPTION,
    url: PAGE_URL,
    type: "website",
    locale: "pt_BR",
    siteName: "Central do Concreto",
  },
};

export default function CobogosPage() {
  return (
    <>
      <section className="bg-cc-black py-20 md:py-28 text-center">
        <div className="mx-auto max-w-3xl px-4">
          <h1 className="text-4xl md:text-5xl font-bold text-white leading-tight mb-4">
            Cobogós de Concreto
          </h1>
          <p className="text-base md:text-lg text-white/90 leading-relaxed max-w-xl mx-auto">
            Peças vazadas que combinam ventilação, iluminação natural e privacidade.
          </p>
        </div>
      </section>

      <section className="py-16 md:py-24 bg-white">
        <div className="mx-auto max-w-3xl px-4 text-center">
          <Eyebrow className="mb-4 justify-center">Em breve</Eyebrow>
          <h2 className="text-2xl md:text-3xl font-medium text-cc-black mb-4">
            Página completa em construção
          </h2>
          <p className="text-base text-cc-gray-600 leading-relaxed mb-8">
            Fabricamos cobogós de concreto pré-moldado em Ivoti-RS · peças vazadas que combinam
            ventilação, iluminação natural e privacidade. Modelos clássicos e contemporâneos pra
            fachadas, muros vazados e divisórias internas. Ficha técnica completa em breve · já está
            no catálogo · pra orçamento ou dúvida fala direto pelo WhatsApp.
          </p>
          <WhatsAppLink
            href={WHATSAPP_URL}
            source="produtos-cobogos-cta"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-cc-whatsapp hover:bg-cc-whatsapp-hover text-white px-7 py-3.5 rounded-full font-medium transition-colors shadow-lg"
          >
            Falar no WhatsApp · (51) 99669-1757
          </WhatsAppLink>
        </div>
      </section>
    </>
  );
}
